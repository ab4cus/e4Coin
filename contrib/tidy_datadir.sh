#!/bin/bash
# Copyright (c) 2013 The dash Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete e4Coin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no e4Coin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old e4Coin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected e4Coin 0.7 datadir."
    ;;
  3)
    echo "Detected e4Coin pre-0.8 datadir."
    ;;
  4)
    echo "Detected e4Coin 0.8 datadir."
    ;;
esac
