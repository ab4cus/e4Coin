...
import RPC call and sends E4C to it. Then other nodes import the addresses,
and the test makes listtransactions and getbalance calls to confirm that the
importing node either did or did not execute rescans picking up the send
transactions.

In the second part of the test, node 0 sends more E4C to each address, and the
...
class ImportRescanTest(e4coinTestFramework):
    def __init__(self):
        super().__init__()
    ...
    def setup_network(self):
        ...
        for i, import_node in enumerate(IMPORT_NODES, 1):
            if import_node.prune:
                # txindex is enabled by default in e4Coin and needs to be disabled for import-rescan.py
