...
def initialize_chain(test_dir, num_nodes, cachedir, extra_args=None, redirect_stderr=False):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            args = [ os.getenv("BITCOIND", "e4coind"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0", "-mocktime="+str(GENESISTIME) ]
            if i > 0:
            ...
            logger.debug("initialize_chain: e4coind started, waiting for RPC to come up")
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False, stderr=None):
    ...
    if binary is None:
        binary = os.getenv("BITCOIND", "e4coind")
...
def assert_start_raises_init_error(i, dirname, extra_args=None, expected_msg=None):
    with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
        try:
            ...
            assert 'e4coind exited' in str(e) #node must have shutdown
            ...
            if expected_msg is None:
                assert_msg = "e4coind should have exited with an error"
            else:
                assert_msg = "e4coind should have exited with expected error " + expected_msg
...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s E4CN too low! (Should be %s E4CN)"%(str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s E4CN too high! (Should be %s E4CN)"%(str(fee), str(target_fee)))
