...
Building with Visual Studio is an alternative to the Linux based [cross-compiler build](https://github.com/e4coin/bitcoin/blob/master/doc/build-windows.md).
...
msbuild /m build_msvc\e4coin.sln /p:Platform=x64 /p:Configuration=Release /t:build
```

Dependencies
---------------------
A number of [open source libraries](https://github.com/e4coin/bitcoin/blob/master/doc/dependencies.md) are required in order to be able to build Bitcoin Core.
...
The [external dependencies](https://github.com/e4coin/bitcoin/blob/master/doc/dependencies.md) required for building are:
...
To build Bitcoin Core without Qt unload or disable the `e4coin-qt`, `libbitcoin_qt` and `test_bitcoin-qt` projects.
...
```
msbuild /m e4coin.sln /p:Platform=x64 /p:Configuration=Release /p:PlatformToolset=v141 /t:build
```

- To build from the command line with the Visual Studio 2019 toolchain use:

```
msbuild /m e4coin.sln /p:Platform=x64 /p:Configuration=Release /t:build
```

- Alternatively open the `build_msvc\e4coin.sln` file in Visual Studio.
...
```
    #- 7z a e4coin-%APPVEYOR_BUILD_VERSION%.zip %APPVEYOR_BUILD_FOLDER%\build_msvc\%platform%\%configuration%\*.exe
    #- path: e4coin-%APPVEYOR_BUILD_VERSION%.zip
```
