...
*Setup instructions for a Gitian build of e4Coin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the e4Coin
...
- [Building e4Coin Core](#building-e4coin-core)
...
Clone the git repositories for e4Coin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/e4cash/e4coin
git clone https://github.com/e4cash/gitian.sigs.git
```
...
in the e4Coin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building e4Coin Core
----------------

To build e4Coin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the e4Coin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/e4coin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/e4cash/e4coin
...
For example:
```bash
URL=https://github.com/crowning-/e4coin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit e4coin=${COMMIT} --url e4coin=${URL} ../e4coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit e4coin=${COMMIT} --url e4coin=${URL} ../e4coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit e4coin=${COMMIT} --url e4coin=${URL} ../e4coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the e4coin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../e4coin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/e4cash/e4coin-detached-sigs.git

BTCPATH=/some/root/path/e4coin
SIGPATH=/some/root/path/e4coin-detached-sigs

./bin/gbuild --url e4coin=${BTCPATH},signature=${SIGPATH} ../e4coin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/e4coin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/e4coin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/e4coin-osx-build.assert
```
...
[e4coin/gitian.sigs](https://github.com/e4cash/gitian.sigs/) repository, or if that's not possible to create a pull
request.
