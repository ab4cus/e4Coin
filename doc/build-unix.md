UNIX BUILD NOTES
====================
Some notes on how to build e4Coin Core in Unix.

(for OpenBSD specific instructions, see [build-openbsd.md](build-openbsd.md))

Base build dependencies
-----------------------
Building the dependencies and e4Coin Core requires some essential build tools and libraries to be installed before.
...
Security
--------
To help make your e4Coin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, e4Coin Core should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, e4Coin Core may be compiled in
...
Add `CC=egcc CXX=eg++ CPP=ecpp` to the dependencies build and the e4Coin Core build:
```bash
$ cd depends
