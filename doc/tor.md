TOR SUPPORT IN E4CN CORE
=======================

It is possible to run e4Coin Core as a Tor hidden service, and connect to such services.
...
1. Run e4Coin Core behind a Tor proxy
----------------------------------

The first step is running e4Coin Core behind a Tor proxy. This will already make all
...
port 9050 and only allows .onion nodes to connect:
	...
	./e4coind -onion=127.0.0.1:9050 -onlynet=tor -listen=0 -addnode=ssapp53tmftyjmjb.onion

In a typical situation, this suffices to run behind a Tor proxy:

	./e4coind -proxy=127.0.0.1:9050


2. Run a e4Coin Core hidden server
-------------------------------
...
your e4coind's P2P listen port (15732 by default).
	...
	-externalip=X   You can tell e4Coin Core about its publicly reachable address using
	                ...
	                /var/lib/tor/e4coincore-service/hostname. Onion addresses are given
...
In a typical situation, where you're only reachable via Tor, this should suffice:
	...
	./e4coind -proxy=127.0.0.1:9050 -externalip=ssapp53tmftyjmjb.onion -listen

(obviously, replace the Onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

	./e4coind ... -bind=127.0.0.1
...
for normal IPv4/IPv6 communication, use:
	...
	./e4coind -onion=127.0.0.1:9050 -externalip=ssapp53tmftyjmjb.onion -discover


3. List of known e4Coin Core Tor relays
------------------------------------
...
e4Coin Core has been updated to make use of this.

This means that if Tor is running (and proper authentication has been configured),
e4Coin Core automatically creates a hidden service to listen on. This will positively 
affect the number of available .onion nodes.

This new feature is enabled by default if e4Coin Core is listening (`-listen`), and
requires a Tor connection to work. It can be explicitly disabled with `-listenonion=0`
and, if not disabled, configured using the `-torcontrol` and `-torpassword` settings.
To show verbose debugging information, pass `-debug=tor`.

Connecting to Tor's control socket API requires one of two authentication methods to be 
configured. For cookie authentication the user running e4coind must have write access 
to the `CookieAuthFile` specified in Tor configuration. In some cases this is 
preconfigured and the creation of a hidden service is automatic. If permission problems 
are seen with `-debug=tor` they can be resolved by adding both the user running tor and 
the user running e4coind to the same group and setting permissions appropriately. On 
Debian-based systems the user running e4coind can be added to the debian-tor group, 
