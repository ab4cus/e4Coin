...
// which are part of the e4Coin repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="e4coin-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          if (hasCache) {
            sh "cd /e4coin-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /e4coin-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /e4coin-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /e4coin-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /e4coin-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /e4coin-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /e4coin-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
