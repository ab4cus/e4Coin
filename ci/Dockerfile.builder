...
# e4coin_hash
RUN git clone https://github.com/e4coinpay/e4coin_hash
...
RUN useradd -u ${USER_ID} -g e4coin -s /bin/bash -m -d /e4coin e4coin
...
RUN mkdir /e4coin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /e4coin-src && \
