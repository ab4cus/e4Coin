...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 E4COIN irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on e4coincentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in e4coin.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!fMnFound) {
        ...
        resultsObj.push_back(Pair("e4coin.conf", statusObj));
...
UniValue gobject_vote_alias(const JSONRPCRequest& request)
{
    ...
    if (deterministicMNManager->IsDeterministicMNsSporkActive()) {
        ...
        if (!pwalletMain->GetKey(dmn->pdmnState->keyIDVoting, votingKey)) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Private key for voting address %s not known by wallet", Ce4coinAddress(dmn->pdmnState->keyIDVoting).ToString()));
        }

        Ce4coinSecret secret(votingKey);
        CMasternodeConfig::CMasternodeEntry mne(dmn->proTxHash.ToString(), dmn->pdmnState->addr.ToStringIPPort(false), secret.ToString(), dmn->collateralOutpoint.hash.ToString(), itostr(dmn->collateralOutpoint.n));
        entries.push_back(mne);
#else
        throw JSONRPCError(RPC_INVALID_PARAMETER, "vote-alias not supported when wallet is disabled");
...
[[ noreturn ]] void gobject_help()
{
    throw std::runtime_error(
            ...
            "  vote-conf          - Vote on a governance object by masternode configured in e4coin.conf\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* e4Coin features */
    { "e4coin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "e4coin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "e4coin",               "gobject",                &gobject,                true,  {} },
    { "e4coin",               "voteraw",                &voteraw,                true,  {} },
