...
void masternode_list_help()
{
    throw std::runtime_error(
            ...
            "  owneraddress   - Print the masternode owner e4Coin address\n"
            "  payee          - Print the masternode payout e4Coin address (can be additionally filtered,\n"
            "                   partial match)\n"
            ...
            "  votingaddress  - Print the masternode voting e4Coin address\n"
        );
}
...
UniValue GetNextMasternodeForPayment(int heightShift)
{
    ...
    Ce4coinAddress payeeAddr;
    if (ExtractDestination(payeeScript, payeeDest)) {
        payeeAddr = Ce4coinAddress(payeeDest);
    }
...
UniValue masternode_genkey(const JSONRPCRequest& request)
{
    ...
    return Ce4coinSecret(secret).ToString();
}
...
UniValue masternode_status(const JSONRPCRequest& request)
{
    ...
    if (deterministicMNManager->IsDeterministicMNsSporkActive()) {
        ...
        if (mnodeman.Get(activeMasternodeInfo.outpoint, mn)) {
            mnObj.push_back(Pair("payee", Ce4coinAddress(mn.keyIDCollateralAddress).ToString()));
        }
...
UniValue masternodelist(const JSONRPCRequest& request)
{
    ...
    if (strMode == "rank") {
        ...
        for (const auto& mnpair : mapMasternodes) {
            ...
            if (ExtractDestination(payeeScript, payeeDest)) {
                payeeStr = Ce4coinAddress(payeeDest).ToString();
            }
            ...
            if (strMode == "activeseconds") {
                ...
                objMN.push_back(Pair("owneraddress", Ce4coinAddress(mn.keyIDOwner).ToString()));
                objMN.push_back(Pair("votingaddress", Ce4coinAddress(mn.keyIDVoting).ToString()));
                ...
                if (strFilter !="" && strOutpoint.find(strFilter) == std::string::npos) continue;
                obj.push_back(Pair(strOutpoint, Ce4coinAddress(mn.keyIDOwner).ToString()));
                ...
                if (strFilter !="" && strOutpoint.find(strFilter) == std::string::npos) continue;
                obj.push_back(Pair(strOutpoint, Ce4coinAddress(mn.keyIDVoting).ToString()));
            }
        }
    }
    return obj;
    ...
    if (strCommand == "decode") {
        ...
        for (const auto& mnb : vecMnb) {
            ...
            if (mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("keyIDCollateralAddress", Ce4coinAddress(mnb.keyIDCollateralAddress).ToString()));
                resultObj.push_back(Pair("keyIDMasternode", Ce4coinAddress(mnb.legacyKeyIDOperator).ToString()));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    { "e4coin",               "masternode",             &masternode,             true,  {} },
    { "e4coin",               "masternodelist",         &masternodelist,         true,  {} },
    { "e4coin",               "masternodebroadcast",    &masternodebroadcast,    true,  {} },
    { "e4coin",               "getpoolinfo",            &getpoolinfo,            true,  {} },
    { "e4coin",               "sentinelping",           &sentinelping,           true,  {} },
#ifdef ENABLE_WALLET
    { "e4coin",               "privatesend",            &privatesend,            false, {} },
#endif // ENABLE_WALLET
};
