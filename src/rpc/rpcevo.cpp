...
std::string GetHelpString(int nParamNum, std::string strParamName)
{
    static const std::map<std::string, std::string> mapParamHelp = {
        {"collateralAddress",
            "%d. \"collateralAddress\"        (string, required) The e4coin address to send the collateral to.\n"
        ...
        {"ownerAddress",
            "%d. \"ownerAddress\"             (string, required) The e4coin address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress",
            "%d. \"payoutAddress\"            (string, required) The e4coin address to use for masternode reward payments.\n"
        },
        {"proTxHash",
...
// Allows to specify e4Coin address or priv key. In case of e4Coin address, the priv key is taken from the wallet
static CKey ParsePrivKey(const std::string &strKeyOrAddress, bool allowAddresses = true) {
    Ce4coinAddress address;
...
{
    Ce4coinAddress address(strAddress);
    CKeyID keyID;
...
void protx_register_fund_help()
{
    throw std::runtime_error(
            ...
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 1000 e4Coin\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (isFundRegister) {
        Ce4coinAddress collateralAddress(request.params[paramIdx].get_str());
    ...
    Ce4coinAddress payoutAddress(request.params[paramIdx + 5].get_str());
    ...
    Ce4coinAddress fundAddress = payoutAddress;
    if (request.params.size() > paramIdx + 6) {
        fundAddress = Ce4coinAddress(request.params[paramIdx + 6].get_str());
        if (!fundAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid e4Coin address: ") + request.params[paramIdx + 6].get_str());
    }
    ...
    if (isFundRegister) {
        ...
        CTxDestination txDest;
        CKeyID keyID;
        if (!ExtractDestination(coin.out.scriptPubKey, txDest) || !Ce4coinAddress(txDest).GetKeyID(keyID)) {
        ...
        if (isPrepareRegister) {
            ...
            ret.push_back(Pair("collateralAddress", Ce4coinAddress(txDest).ToString()));
            ...
            if (!pwalletMain->GetKey(keyID, key)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("collateral key not in wallet: %s", Ce4coinAddress(keyID).ToString()));
            }
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() >= 5) {
        if (request.params[4].get_str().empty()) {
            ...
            Ce4coinAddress payoutAddress(request.params[4].get_str());
            if (!payoutAddress.IsValid()) {
    ...
    if (request.params.size() >= 6) {
        Ce4coinAddress feeSourceAddress = Ce4coinAddress(request.params[5].get_str());
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid e4Coin address: ") + request.params[5].get_str());
...
UniValue protx_update_registrar(const JSONRPCRequest& request)
{
    ...
    if (!pwalletMain->GetKey(dmn->pdmnState->keyIDOwner, keyOwner)) {
        throw std::runtime_error(strprintf("Private key for owner address %s not found in your wallet", Ce4coinAddress(dmn->pdmnState->keyIDOwner).ToString()));
    }
    ...
    Ce4coinAddress feeSourceAddress = payoutAddress;
    if (request.params.size() > 5) {
        feeSourceAddress = Ce4coinAddress(request.params[5].get_str());
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid e4Coin address: ") + request.params[5].get_str());
    }
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 4) {
        Ce4coinAddress feeSourceAddress = Ce4coinAddress(request.params[4].get_str());
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid e4Coin address: ") + request.params[4].get_str());
        FundSpecialTx(tx, ptx, feeSourceAddress.Get());
