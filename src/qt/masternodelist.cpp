...
void MasternodeList::updateMyMasternodeInfo(QString strAlias, QString strAddr, const COutPoint& outpoint)
{
    ...
    QTableWidgetItem* pubkeyItem = new QTableWidgetItem(QString::fromStdString(fFound ? Ce4coinAddress(infoMn.keyIDCollateralAddress).ToString() : ""));
...
void MasternodeList::updateNodeList()
{
    ...
    for (const auto& mnpair : mapMasternodes) {
        ...
        QTableWidgetItem* pubkeyItem = new QTableWidgetItem(QString::fromStdString(Ce4coinAddress(mn.keyIDCollateralAddress).ToString()));
...
void MasternodeList::updateDIP3List()
{
    ...
    mnList.ForEachMN(false, [&](const CDeterministicMNCPtr& dmn) {
        ...
        if (ExtractDestination(dmn->pdmnState->scriptPayout, payeeDest)) {
            payeeStr = QString::fromStdString(Ce4coinAddress(payeeDest).ToString());
        } else {
            payeeStr = tr("UNKNOWN");
        }
        ...
        if (dmn->nOperatorReward) {
            ...
            if (dmn->pdmnState->scriptOperatorPayout != CScript()) {
                ...
                if (ExtractDestination(dmn->pdmnState->scriptOperatorPayout, operatorDest)) {
                    operatorRewardStr += tr("to %1").arg(QString::fromStdString(Ce4coinAddress(operatorDest).ToString()));
                } else {
...
void MasternodeList::ShowQRCode(std::string strAlias)
{
    ...
    if (fFound) {
        ...
        strHTML += "<b>" + tr("Payee") +        ": </b>" + GUIUtil::HtmlEscape(Ce4coinAddress(mn.pubKeyCollateralAddress.GetID()).ToString()) + "<br>";
