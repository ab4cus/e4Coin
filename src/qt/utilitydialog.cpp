    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    if (helpMode == about)
    {
        ...
        QString header = tr("Usage:") + "\n" +
            "  e4coin-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_E4COIN_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", e4coinGUI::DEFAULT_UIPLATFORM));
        }
...
All the e4Coin in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.001 E4COIN, 0.01 E4COIN, 0.1 E4COIN, 1 E4COIN and 10 E4COIN -- sort of like the paper money you use every day.</li> \
...
For more information, see the <a href=\"https://docs.e4coin.org/en/latest/wallets/e4coincore/privatesend-instantsend.html\">PrivateSend documentation</a>."
        ));
        ui->aboutMessage->setWordWrap(true);
        ui->helpMessage->setVisible(false);
        ui->aboutLogo->setVisible(false);
...
QWidget *ShutdownWindow::showShutdownWindow(e4coinGUI *window)
{
    if (!window)
        return nullptr;
