...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), E4COIN_CONF_FILENAME));
    if (mode == HMM_E4COIND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), E4COIN_PID_FILENAME));
#endif
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "e4coin (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_E4COIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all e4Coin specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N E4COIN anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/e4cash/e4Coin>";
    const std::string URL_WEBSITE = "<https://e4coin.io>";
...
/** Sanity checks
 *  Ensure that e4Coin Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("e4Coin Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single e4Coin Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (!fLogTimestamps)
        ...
        LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", E4COIN_CONF_FILENAME)).string());
    ...
    // lite mode disables all e4Coin-specific functionality
    fLiteMode = GetBoolArg("-litemode", false);
    LogPrintf("fLiteMode %d\n", fLiteMode);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all e4Coin-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/e4coinpay/e4coin/pull/1817 and https://github.com/e4coinpay/e4coin/pull/1743
    ...
    if(fMasternodeMode) {
        ...
        if(!strMasterNodePrivKey.empty()) {
            ...
            LogPrintf("  keyIDOperator: %s\n", Ce4coinAddress(activeMasternodeInfo.legacyKeyIDOperator).ToString());
        } else {
    ...
    // ********************************************************* Step 11c: schedule e4Coin-specific tasks
